...
//asfalhs metatroph string se int
...
//asfalhs dynamic desmeysh mnhmhs (stack)
...
//diagnwstiko mhnyma ston xrhsth gia lathos arithmo orismatwn
...
void *thread_start_fn(void *arg) {
        ...
        fprintf(stderr, "Thread with Thread_id: %d of %d has just STARTED.\n", thr->thrid, thr->thrcnt);

        for (i = thr->thrid; i < thr->len; i += thr->thrcnt)
                thr->arr[i] *= thr->mul;

        fprintf(stderr, "Thread with Thread_id: %d of %d has just ENDED.\n", thr->thrid, thr->thrcnt);
...
int main(int argc, char *argv[]) {
        ...
        for (i = 0; i < arrsize; i++) {
                arr[i] = VAL1;              //VAL 1 in each position
        }

        //Multiply it by VAL2 using thrcnt threads, in parallel
        
        //MAKE SPACE FOR THREADS STRUCT INSTANCES (MALLOC)
        ...
        for (i = 0; i < thrcnt; i++) {
                ...
                // Spawn new thread    ID      DEF.ATTR   START FUNC     ITS ARG (THREAD INFO)         
                ret = pthread_create(&thr[i].tid, NULL, thread_start_fn, &thr[i]);
                if (ret) {
                        perror_pthread(ret, "pthread_create");      //START FUNC: MULT EACH BY VAL2
                        exit(1);
                }
        }
        ...
        for (i = 0; i < thrcnt; i++) {
                ...
                if (ret) {      // because normally returns 0
