...
void *increase_fn(void *arg) {
        ...
        for (i = 0; i < N; i++) {
                if (USE_ATOMIC_OPS) {			// 1st way with atomic ops
                         __sync_add_and_fetch(ip, 1);
                } 
		else {					// 2nd way with mutexes
...
int main(int argc, char *argv[]) {
	...
	val = 0;
       ...
       // Create threads
        ...
        ret = pthread_create(&t1, NULL, increase_fn, &val);	// create the 1st thread with strat_fun = increase_fn
        if (ret) {
        ...
        ret = pthread_create(&t2, NULL, decrease_fn, &val);	// create the 2nd thread with start_fun = decreaxe_fn
        if (ret) {
                perror_pthread(ret, "pthread_create");
                exit(1);
        }
